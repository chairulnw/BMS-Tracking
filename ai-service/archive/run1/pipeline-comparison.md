...
- **Akurasi** —  proporsi orang GT yang dapet **tepat 1** identitas prediksi, DAN identitas
  itu **cuma dipakai dia sendiri** (nggak ke-*merge*/*split* sama sekali). Beda dari Precision/Recall/F1 (per pasangan) — ini per **orang**, jadi bisa beda urutan rangking dari F1 (kombinasi F1 tinggi belum tentu akurasi tertinggi, tergantung sebaran kesalahannya).
...
| #   | Detektor   | Tracker   | Re-ID                 | Status  | Cakupan deteksi | Identitas terbentuk | Precision | Recall | F1    | Akurasi      | False merge | False split | FPS efektif | CPU peak | RAM peak | Decode ms | Detect ms | Track ms | ReID ms | Match ms | Total AI ms |
| --- | ---------- | --------- | --------------------- | ------- | --------------- | ------------------- | --------- | ------ | ----- | ------------ | ----------- | ----------- | ----------- | -------- | -------- | --------- | --------- | -------- | ------- | -------- | ----------- |
...
| 3   | YOLO26n    | ByteTrack | BoT (ResNet50)†       | Selesai | 190/259 | 20/11 | 0.711 | 0.607 | 0.655 | 1/11 (9.1%) | 4 | 9 | 7.63 | 23% | 80% |  |  |  |  |  |  |
...
**†/‡ BoT (ResNet50):** dua checkpoint beda, jangan dibandingin apple-to-apple.
- **† (#3)** — checkpoint resmi paper (Luo et al., *Bag of Tricks and a Strong
  Baseline for Deep Person Re-ID*, CVPRW 2019, arXiv:1903.07071), Rank-1 94.5%
  di Market1501. Link resmi di README repo
  ...
  repo yang sama (`market_resnet50_model_120_rank1_945.pth`), diverifikasi
  cocok struktur key-nya sama `Baseline.load_param()` kode asli. Checkpoint
  ini SEHAT (bukan degenerate), tapi hasilnya di data CCTV kita tetap kalah
  dari OSNet & TransReID (F1 0.655 vs 0.902/0.949) — lihat Kesimpulan.
- **‡ (#6,9,12,15,18,21,24,27)** — checkpoint lama dari model zoo `torchreid`
  (`resnet50_market1501_converted.pth`), rank1 asli cuma 25.67% (undertrained,
  bukan bug konversi — diverifikasi baca metadata checkpoint-nya langsung).
  DEGENERATE: semua orang ke-gabung jadi 1 identitas (`Identitas terbentuk`
  selalu `1/11`, precision ~0.1). Belum di-rerun pakai checkpoint † karena
  hasil #3 udah cukup nunjukkin BoT kalah — lihat metodologi (7 kombinasi
  prioritas, full grid cuma kalau ada indikasi *interaction effect*).
...
- **#3 (YOLO26n + ByteTrack + BoT-ResNet50, checkpoint resmi†)** — sempat
  dicurigai kombinasi #3-#27 (BoT-ResNet50) di tabel awal 0% akurasi karena
  checkpoint korup, ternyata benar setelah dicek: rank1 asli cuma 25.67%
  (undertrained), bukan bug sistem. Diuji ulang pakai checkpoint resmi paper
  (Rank-1 94.5% di Market1501) — hasilnya sehat (precision 0.711, bukan lagi
  ~0.1) tapi F1 tetap kalah jauh dari OSNet & TransReID (0.655 vs
  0.902/0.949, Akurasi cuma 9.1%). Mengonfirmasi BoT-ResNet50 bukan kandidat
  layak di data CCTV ini, terlepas dari kualitas checkpoint-nya.
